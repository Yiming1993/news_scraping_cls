...
class report_extraction(object):
    ...
    def report_making_alg1(self,date,startdate_true,startdate_false,enddate):
        ...
        if y_test and all_predictions is not None:
            ...
            print("Accuracy: {:g}".format((Tp + Tn) / (Tp + Tn + Fp + Fn)))
            print("Precision:{:g}".format(Tp / (Tp + Fp)))
            print("Recall:{:g}".format(Tp / (Tp + Fn)))
            print("Specificity:{:g}".format(Tn / (Tn + Fp)))
    ...
    def headline_report(self,date):
        y_pred_pre = {}
        cover_list = {}
        news_pred = self.db.NEWS.find({'$and': [{"coll_date": date}, {"hd_alg1":{"$exists":True}}]})
        for doc in news_pred:
            y_pred_pre[doc["title"]] = doc["hd_alg1"]
...
if __name__ == '__main__':
    ...
    D.report_making_alg1(date, startdate_true, startdate_false, enddate)
    D.report_making_alg2(date, startdate_true, startdate_false, enddate)
    D.tag_run(date)
